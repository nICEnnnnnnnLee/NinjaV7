...
class FadeInImage extends StatelessWidget {
  ...
  FadeInImage.memoryNetwork({
    ...
    String alterPath,
        ...
        image = MyNetworkImage(image, scale: imageScale,sdCache: sdcache, needRefresh:needRefresh, seed:seed,alterPath: alterPath,findProxy: findProxy),
        super(key: key);
...
class MyNetworkImage extends ImageProvider<MyNetworkImage>{
  ...
  const MyNetworkImage(this.url, { this.scale = 1.0, this.headers,this.sdCache, this.needRefresh, this.seed, this.alterPath,this.findProxy })
      : assert(url != null),
        assert(scale != null);
...
//            print(alterPath);
      try{
        if(alterPath != null){
          final Uint8List bytes =await  _getFromAlterPath(alterPath);
...
//      request.headers.set("Accept-Language", "zh-CN,zh;q=0.8,zh-TW;q=0.7,zh-HK;q=0.5,en-US;q=0.3,en;q=0.2");
      ...
      if(sdCache&&bytes.lengthInBytes != 0){
  ...
  bool operator ==(dynamic other) {
    ...
    // 人工刷新判断 当需要人工刷新时，还需要判断seed是否相同
    if(needRefresh != typedOther.needRefresh)
      return false;
    if(needRefresh && seed != typedOther.seed)
      return false;
...
//    print("path =${path}");
  ...
  _getFromAlterPath(String alterPath) async{
    var file = File(alterPath);
    bool exist =  await file.exists();
    if(exist){
      final Uint8List bytes = await file.readAsBytes();
      return bytes;
